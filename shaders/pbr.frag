...
layout(std140, binding = 3) uniform PerFrameDataBlock {
    ...
    ivec4 frameFlags; // x: light count, y: debug flag, z: debug target, w: use IBL
    vec4 envParams;   // x: env intensity, y: prefilter mip levels
} uFrame;
...
layout(std140, binding = 4) uniform ObjectDataBlock {
    mat4 model;
    mat4 normalMatrix;
    ivec4 materialFlags; // x: material index, y: has tangents, z: has primary UVs, w: has secondary UVs
    ivec4 textureUsage;  // x: albedo, y: metallicRoughness, z: normal, w: ao
    ivec4 textureUsage2; // x: emissive, y: alphaMode, z,w: reserved
    ivec4 uvSets0;       // x: albedo, y: metallicRoughness, z: normal, w: ao
    ivec4 uvSets1;       // x: emissive, y/z/w: reserved
} uObject;
...
struct MaterialData {
    ...
    vec4 pbrParams;        // x: metallic, y: roughness, z: ao, w: ao intensity
    vec4 extraParams;      // x: normalScale, y: normalStrength, z: alphaCutoff, w: shininess
    ivec4 textureUsage;    // x: albedo, y: metallicRoughness, z: normal, w: ao
    ivec4 textureUsage2;   // x: emissive, y: occlusionFromMR, z: unlit, w: usePBR
    ivec4 uvSets0;
    ivec4 uvSets1;         // x: emissive uv, y: doubleSided, z: alphaMode, w: padding
...
// Optional: user-provided height map (not bound by default). If uHasHeightMap=false, shader will not sample it.
layout(binding = 5) uniform sampler2D uHeightMap;

layout(binding = 16) uniform samplerCube uIrradianceMap;
layout(binding = 17) uniform samplerCube uPreFilterMap;
layout(binding = 18) uniform sampler2D  uBRDFLut;
...
float computeDistanceAttenuation(GpuLight light, float distance)
{
    ...
    float attenuation = 1.0 / max(denom, EPS);
...
{
    // 1) Prefer a dedicated height map when available
    if (uHasHeightMap)
        return texture(uHeightMap, uv).r;

    // 2) Optionally derive height from the normal map when requested
    if (canUseNormalAlpha) {
        vec4 n = texture(uNormalMap, uv);
        float a = n.a;

        // Heuristic: treat alpha as invalid height if near-constant (close to 0 or 1)
        // or with negligible variation across neighboring pixels.
...
vec3 computeNormal(vec2 uv, bool useNormalMap, float strength, int hasTangents)
{
    ...
    vec3 tangentNormal = decodeNormal(texture(uNormalMap, uv).xyz);
...
vec3 evaluateBRDF(vec3 N, vec3 V, vec3 L, float NdotV, vec3 F0,
    vec3 albedo, float metallic, float roughness, vec3 radiance)
{
    float NdotL = max(dot(N, L), 0.0);
...
vec3 evaluateGpuLight(GpuLight light, vec3 fragPos, vec3 N, vec3 V, float NdotV,
    vec3 F0, vec3 albedo, float metallic, float roughness)
{
    ...
    vec3 radiance = baseColor * intensity;
    ...
    return evaluateBRDF(N, V, L, NdotV, F0, albedo, metallic, roughness,
        radiance * attenuation * spot * shadow);
}
...
void main()
{
    ...
    bool materialUsePBR = material.textureUsage2.w != 0;
    bool unlit = material.textureUsage2.z != 0;

    // Use only uObject.textureUsage to avoid dual-gating issues
    // CPU-side already validates texture availability before binding
    bool useAlbedoMap = (uObject.textureUsage.x != 0);
    bool useMetallicRoughnessMap = (uObject.textureUsage.y != 0);
    bool useNormalMap = (uObject.textureUsage.z != 0);
    bool useAOMap = (uObject.textureUsage.w != 0);
    bool useEmissiveMap = (uObject.textureUsage2.x != 0);
    bool occlusionFromMR = material.textureUsage2.y != 0;
    ...
    vec2 metallicRoughnessUV = transformedUV(uObject.uvSets0.y, material.uvTransformMR, material.uvRotations.y);
    ...
    // Optional parallax mapping: compute once from the normal UV space and apply to all
    if (uParallaxEnabled) {
        // LearnOpenGL tutorial parallax approach
        ...
        metallicRoughnessUV = ParallaxMapping(metallicRoughnessUV, viewDir, canUseNormalAlpha);
        aoUV = ParallaxMapping(aoUV, viewDir, canUseNormalAlpha);
        emissiveUV = ParallaxMapping(emissiveUV, viewDir, canUseNormalAlpha);
    }

    vec3 albedo = clamp(material.baseColor.rgb, vec3(0.0), vec3(1.0));
    float alpha = clamp(material.baseColor.a, 0.0, 1.0);

    if (useAlbedoMap) {
        vec4 tex = texture(uAlbedoMap, albedoUV);
        // Prevent pure black from zeroing out lighting
        albedo *= max(tex.rgb, vec3(0.001));
        alpha *= tex.a;
    }
    ...
    vec3 emissiveSample = useEmissiveMap ? texture(uEmissiveMap, emissiveUV).rgb : vec3(1.0);
    vec3 emissive = emissiveColor * emissiveSample * emissiveIntensity;
    ...
    float aoSample = 1.0;
    float occlFromMR = 1.0;
    if (useMetallicRoughnessMap) {
        vec3 mr = texture(uMetallicRoughnessMap, metallicRoughnessUV).rgb;
        metallic = clamp(mr.b, 0.0, 1.0);
        roughness = clamp(mr.g, 0.04, 1.0);
        if (occlusionFromMR)
            occlFromMR = clamp(mr.r, 0.0, 1.0);
    }

    if (useAOMap)
        aoSample = clamp(texture(uAOMap, aoUV).r, 0.0, 1.0);
    else if (occlusionFromMR && useMetallicRoughnessMap)
        aoSample = occlFromMR;
    ...
    if (lightCount > 0) {
        for (int i = 0; i < lightCount; ++i) {
            directLighting += evaluateGpuLight(uLights[i], FragPos, N, V, NdotV, F0, albedo, metallic, roughness);
        }
    } else {
        vec3 fallbackDir = uFrame.lightPos.xyz - FragPos;
        float dist = length(fallbackDir);
        if (dist > EPS) {
            vec3 L = fallbackDir / dist;
            float attenuation = 1.0 / max(dist * dist, EPS);
            vec3 radiance = uFrame.lightColor.rgb * attenuation;
            directLighting += evaluateBRDF(N, V, L, NdotV, F0, albedo, metallic, roughness, radiance);
        }
    }

    bool useIBL = (uFrame.frameFlags.w != 0);
    float envIntensity = uFrame.envParams.x;
    ...
    if (useIBL) {
        ...
        // for IBL diffuse we only remove the metallic part,
        // NOT the view-dependent Fresnel
        vec3 kD_ibl = vec3(1.0 - metallic);
        ...
        float maxPrefilterMip = max(uPrefilterMipCount, 0.0);
        if (maxPrefilterMip <= 0.0)
            maxPrefilterMip = max(uFrame.envParams.y, 0.0);
        ...
        // 3) apply AO + env intensity
        iblDiffuse  = diffuseIbl * ao * envIntensity;
        iblSpecular = specularIbl * envIntensity;
    }
    ...
    if (!useIBL) {
        ...
        ambient = ambientColor * ambientStrength * albedo * ao;
    }

    vec3 color = ambient + directLighting + iblDiffuse + iblSpecular + emissive;

    int debugFlag = uFrame.frameFlags.y;
    int debugTarget = uFrame.frameFlags.z;
    if (debugFlag != 0) {
        if (debugTarget == 0) color = albedo;
        else if (debugTarget == 1) color = vec3(NdotV);
        else if (debugTarget == 2) color = vec3(metallic);
        else if (debugTarget == 3) color = vec3(roughness);
        else if (debugTarget == 4) color = vec3(ao);
        else if (debugTarget == 5) color = emissive;
        else if (debugTarget == 6) {
            // Visualize height (prefer dedicated height map) using the normal UV domain
